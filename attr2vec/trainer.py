...
def train():
    ...
    batch_size, epochs, base_lr, warmup_epochs = 2048, 500, 2e-4, 20
    
    model = DDP(NaturalResourceFoundationModel(config).cuda(local_rank), device_ids=[local_rank], find_unused_parameters=True)
    ...
    for epoch in range(epochs):
        ...
        if is_master and total_b > 0:
            ...
            print(f"📈 Ep {epoch+1:03d}/500 | Loss: {avg_loss:.6f} | LR: {curr_lr:.2e} | T: {time.time()-start_time:.1f}s")
