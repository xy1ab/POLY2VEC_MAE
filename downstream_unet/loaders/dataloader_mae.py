...
class PolyMAEDataset(Dataset):
    """
    负责加载 .pt 格式的 Polygon 三角形列表，并支持实时数据增强。
    """
    def __init__(self, pt_path, geom_type='polygon', augment_times=1):
        super().__init__()
        self.data_list = torch.load(pt_path) # List of numpy arrays [N, 3, 2]
