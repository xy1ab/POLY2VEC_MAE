...
def resolve_precision_for_device(device: torch.device | str, precision: str) -> str:
    ...
    if dev.type != "cuda" and precision in {"fp16", "bf16"}:
        return "fp32"

    if dev.type == "cuda" and precision == "bf16" and not torch.cuda.is_bf16_supported():
        return "fp16"
...
def should_enable_grad_scaler(device: torch.device | str, precision: str) -> bool:
    ...
    Returns:
        True only when running fp16 on CUDA.
    """
    dev = torch.device(device)
    resolved = resolve_precision_for_device(dev, precision)
    return dev.type == "cuda" and resolved == "fp16"


def build_grad_scaler(device: torch.device | str, precision: str) -> torch.amp.GradScaler | torch.cuda.amp.GradScaler:
    ...
    Notes:
        ...
        versions only provide `torch.cuda.amp.GradScaler(...)`. This helper
...
def autocast_context(device: torch.device | str, precision: str) -> ContextManager:
    ...
    dev = torch.device(device)
    resolved = resolve_precision_for_device(dev, precision)
    if dev.type != "cuda" or resolved == "fp32":
        return nullcontext()
    return torch.autocast(device_type="cuda", dtype=precision_to_torch_dtype(resolved))
