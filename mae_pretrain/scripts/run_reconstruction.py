...
class Res_Dataset(Dataset):
    def __init__(self, data):
        self.input = data['embedding'].detach().cpu().float()
        self.label = data['imag'].detach().cpu().float()
    def __len__(self):
        return len(self.label)

    def __getitem__(self, idx):
        # 读取时转回 float32 供模型计算
        return self.input[idx].float(), self.label[idx].float()
...
def train(args):
    ...
    model = TransUNetdecoder(embed_dim=384)
