...
def _load_resume_config(resume_dir: str | Path, project_root: Path) -> tuple[dict, Path]:
    ...
    if __package__ in {None, ""}:
        ...
        load_yaml_config = importlib.import_module("mae_pretrain.src.utils.config").load_yaml_config
    else:
...
def main() -> None:
    ...
    if __package__ in {None, ""}:
        ...
        run_cli = importlib.import_module("mae_pretrain.src.engine.trainer").run_cli
